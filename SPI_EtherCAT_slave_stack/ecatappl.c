...
------
-----------------------------------------------------------------------------------------*/
#include "../SPI_EtherCAT_slave_stack/ecatslv.h"

#define   _ECATAPPL_ 1
#include "../SPI_EtherCAT_slave_stack/ecatappl.h"
#undef    _ECATAPPL_
...
void PDO_InputMapping(void)
{
    ...
    //数组共定义了32个字
    //此在input数中由于在第一个控制字节中是16位后，中间空一个16位的数据，导致后面long型变量数据读取失败
//    aPdInputData[1]=aPdInputData[2];//control word
//    aPdInputData[2]=aPdInputData[3];//  p
//    aPdInputData[3]=aPdInputData[4];//
//    aPdInputData[3]=aPdInputData[4];//  v
//    aPdInputData[5]=aPdInputData[6];//
//    aPdInputData[6]=aPdInputData[7];//  mode
