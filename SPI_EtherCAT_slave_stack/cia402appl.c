...
#include "../SPI_EtherCAT_slave_stack/applInterface.h"
#include "../SPI_EtherCAT_slave_stack/coeappl.h"
#include "../SPI_EtherCAT_slave_stack/ecat_def.h"

#define _CiA402_
#include "../SPI_EtherCAT_slave_stack/cia402appl.h"
#undef _CiA402_
...
//                pInputs->ObjModesOfOperationDisplay = SWAPWORD((LocalAxes[AxisIndex].Objects.objModesOfOperationDisplay & 0x00FF));
              //数据传递过程中转换成结构体后数据会错位，修改为直接数组访问赋值
                Size = SIZEOF(TCiA402PDO1A00);

                pTmpData[0+j*Size] = SWAPWORD(LocalAxes[AxisIndex].Objects.objStatusWord);
                //long型变量取低16位
                pTmpData[1+j*Size] = SWAPDWORD(LocalAxes[AxisIndex].Objects.objPositionActualValue);
                //long型变量取高16位
                pTmpData[2+j*Size] = SWAPDWORD(LocalAxes[AxisIndex].Objects.objPositionActualValue>>16);
                //long型变量取低16位
                pTmpData[3+j*Size] = SWAPDWORD(LocalAxes[AxisIndex].Objects.objVelocityActualValue);
                //long型变量取高16位
                pTmpData[4+j*Size] = SWAPDWORD(LocalAxes[AxisIndex].Objects.objVelocityActualValue>>16);

                pTmpData[5+j*Size] = SWAPWORD((LocalAxes[AxisIndex].Objects.objModesOfOperationDisplay & 0x00FF));
...
\brief    This function will copies the outputs from the ESC memory to the local memory
            to the hardware
                           此功能将把ESC存储器的输出复制到本地存储器到硬件(数据传递过程中出现数据传递失败问题)
*////////////////////////////////////////////////////////////////////////////////////////
...
//            LocalAxes[AxisIndex].Objects.objModesOfOperation = SWAPWORD((pOutputs->ObjModesOfOperation & 0x00FF));
             //数据传递过程中转换成结构体后数据会错位，修改为直接数组访问赋值
            Size = SIZEOF(TCiA402PDO1600);
            LocalAxes[AxisIndex].Objects.objControlWord = SWAPWORD((UINT16)pData[0+j*Size]);
            LocalAxes[AxisIndex].Objects.objTargetPosition = SWAPDWORD(pData[1+j*Size]|(UINT32)pData[2+j*Size]<<16);
            LocalAxes[AxisIndex].Objects.objTargetVelocity    = SWAPDWORD(pData[3+j*Size]|(UINT32)pData[4+j*Size]<<16);
            LocalAxes[AxisIndex].Objects.objModesOfOperation = SWAPWORD((pData[5+j*Size] & 0x00FF));
            /*shift pointer PDO mapping object following*/
...
//         LocalAxes[AxisIndex].Objects.objTargetVelocity  = SWAPDWORD(pOutputs->ObjTargetVelocity);
         //数据传递过程中转换成结构体后数据会错位，修改为直接数组访问赋值
         Size = SIZEOF(TCiA402PDO1602);
         LocalAxes[AxisIndex].Objects.objControlWord = SWAPWORD((UINT16)pData[0+j*Size]);
         LocalAxes[AxisIndex].Objects.objTargetVelocity  = SWAPDWORD(pData[1+j*Size]|(UINT32)pData[2+j*Size]<<16);
